...
The `Walk-structure` directive causes the SaveAsXML processor to walk the first-level structural elements (Kids array of the StructRoot) of the tagged PDF document to be saved. For more information, see [Walk-children](index.md#50409487_34099).

Structural elements are traversed in the order found in the logical structure tree. An event is generated on entering and on exiting each structural element. The event-list specified by the `Use-event-list` attribute of the `Walk-structure` directive is searched for a matching `Event` directive. For more information, see [Define-event-list](index.md#50409487_51177).
