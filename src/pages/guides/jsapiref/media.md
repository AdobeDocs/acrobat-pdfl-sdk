...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | `MediaPlayer` object | R |
...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | `ScreenAnnot` object | R/W |
...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | `MediaSettings` object | R/W |
...
| close | play | stop |
| open | seek | triggerGetRect |
| pause | setFocus | where |
...
If the player has the keyboard focus, a Blur event (`onBlur` or `afterBlur`) is triggered before the Close event. Other events, such as Status (`onStatus` or `afterStatus`) and Stop (`onStop` or `afterStop`), may also be triggered depending on the particular media player.
...
If `MediaPlayer.settings.autoPlay` is `true` (the default), playback begins and a Play event (`onPlay` or `afterPlay`) is triggered.

The `open` method may result in a security prompt dialog box, depending on the user’s settings. It may also result in events being triggered in objects such as other media players and screen annotations. For example, if another media player has the keyboard focus, it will receive a Blur event (`onBlur`  or `afterBlur`).
...
Pauses playback of the current media and triggers a Pause event (`onPause` or `afterPause`). The Pause event may occur during the `pause` call or afterward, depending on the player.
...
Example: See  seek for an example of usage.
...
Starts playback of the current media and triggers a Play event (`onPlay` or `afterPlay`). The Play event may occur during the `play` call or afterward, depending on the player.
...
Example: See Example 2 after :ref:`seek.
...
Most, but not all, media players trigger a Seek event (`onSeek` or `afterSeek`) when a seek is completed.
...
Sets the keyboard focus to the media player and triggers a Focus event (`onFocus` or `afterFocus`). If another player or PDF object has the focus, that object receives a Blur event (`onBlur` or `afterBlur`). If the media player already has the focus, nothing happens. If the player is not open or not visible, an exception is thrown.
...
Stops playback of the current media, if it is playing or paused, and triggers a Stop event (`onStop` or `afterStop`). The Stop event may occur during execution of the `stop` method or afterward, depending on the player. Does nothing if the media is not playing or paused.
...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | `Rendition` object | R |
...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | Array of `MediaReject` objects | R |
...
Example: See the examples following afterReady and players.
...
Example: See the examples that follow `app.media.` getURLData.
...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | `MediaOffset` object | R/W |
...
| `over` | Number | Specifies the window to which the floating window is to be aligned. The value of `over` is one of the values of `app.media.over`. |
...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | Monitor or Monitors object | R/W |
...
| Version (Key) | Save-Prefs | Security | Product | Type | Access |
| 6.0 | No | No | All | Players or Array of String | R/W |
...
Example: See the example that follows endAt.
