...
4. Create a `PDAnnot` object by invoking the `PDPageCreateAnnot` method and passing the following arguments:
   ...
   - An `ASAtom` object that represents the annotation’s subtype. Because a text annotation is created, specify `Text` as the annotation’s subtype. (See the [Acrobat and PDF Library API Reference](https://www.adobe.com/go/apireference).)
...
2. Get the redaction annotation’s properties, by invoking the PDRedactionGetProps function. The first argument is the pointer to the redaction annotation obtained in Step Obtain a pointer to the redaction annotation. (See Retrieving existing annotations.) Ensure that the pointer references an annotation with the ``Subtype` value `Redact`. <Plugins\_Annotations.html#50618420\_94111>`\_\_ and the second argument is a pointer to the `PDRedactParamsRec` structure.
