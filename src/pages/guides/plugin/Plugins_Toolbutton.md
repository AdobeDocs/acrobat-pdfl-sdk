...
- An `ASAtom` object that represents the button name. For information about button names, see the [Acrobat and PDF Library API Reference](https://www.adobe.com/go/apireference).
...
> **Note**
>
> Notice that the application logic that creates a toolbar button is located in the `PluginInit` procedure. (See About plugin initialization.)
