...
3. Change ACRO\_SDK\_LEVEL to 0x000A0000 from 0x00090000
4. Remove the `AcroColorHFT` define by removing the following line from the file:

`/D PI_ACROCOLOR_VERSION=0x00060000`

1. Add the READER\_PLUGIN define by adding the following to the file:

`/D READER_PLUGIN`

1. Save the `AcroDspOptions.rsp` file.
2. Open and rebuild your Visual C++ plugin project.
...
The MakeKeys tool is a command line (Terminal tool). You can find this tool in the PluginSupport/Tools/Reader-enabling Tools directory in the Acrobat SDK. You can also download a ZIP file that contains this tool from [http://www.adobe.com/go/rikla\_tools](http://www.adobe.com/go/rikla_tools).
...
The following example command line shows the options and parameters you supply to the MakeKeys tool. Replace the placeholders with a meaningful string:
...
1. Submit the public key file to Adobe using the PDF form that you received by email, as described in readerplugin.
...
The SignPlugin tool is a command line (Terminal tool). You can find this tool in the PluginSupport/Tools/Reader-enabling Tools directory in the Acrobat SDK. You can also download a ZIP file that contains this tool from [http://www.adobe.com/go/rikla\_tools](http://www.adobe.com/go/rikla_tools).
...
3. Run the SignPlugin tool, providing it with the following parameters:
   - `-kp` option followed by the location of the file that contains the key pair. This file was produced by the MakeKeys tool. (See kp)
   - `-cf` option followed by the location of the file that contains the Reader Integrated Key provided by Adobe. (See cf)
...
The plugin can now be loaded by Adobe Reader. If you encounter any difficulties, see Troubleshooting an Adobe Reader plugin.
...
In MacOS, for the plugin to be notarized after enabling the plugin for Adobe Reader, the SignPlugin utility can be used in the following manner -
...
Here, team\_id refers to the 10-character unique ID assigned by Apple in Apple Developer Certificate.

If SignPlugin is successful, it returns the message: “Plugin.acroplugin successfully Reader Enabled.”

> **Note**
>
> Reader.cer and Reader.dig files should be generated in /MyDirectory/MyPlugin/Contents/Resources. If not generated there, please move the files from /MyDirectory/MyPlugin/Contents/MacOS to /MyDirectory/MyPlugin/Contents/Resources.

The plugin can now be codesigned and sent for Notarization. If you encounter any difficulties, see Troubleshooting an Adobe Reader plugin.
...
With this symbol defined, only those HFTs available in Adobe Reader are acquired. For information about an HFT, see Working with Host Function Tables.
...
The first error is displayed if the plugin returns `false` from the `PISetupSDK` method (defined in PIMain). The method returns `false` if the plugin attempts to acquire an HFT that is not available. (See Working with Host Function Tables.)
