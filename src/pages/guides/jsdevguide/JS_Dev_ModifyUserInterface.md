...
There is brief example of `app.addSubMenu` and `app.addMenuItem`, see [Executing privileged methods through the menu](JS_Dev_Contexts.md#40133).

The Example [Adding a menu item](JS_Dev_ModifyUserInterface.md#95641) installs a menu item under the main Tools menu. When executed, the menu calls the function `loadATBToolButton()`. This function loads the custom toolbar set, the definition of which follows.
...
For Acrobat 8, this script assumes that the Enable Global Object Security Policy is enabled in the JavaScript section of the Preferences, see the discussion in [Enable the global object security policy](JS_Dev_AcrobatForms.md#43014).
...
- [Thumbnails](JS_Dev_ModifyUserInterface.md#35265)
- [Bookmarks](JS_Dev_ModifyUserInterface.md#14709)
- [Links](JS_Dev_ModifyUserInterface.md#23951)
- [Using actions for special effects](JS_Dev_ModifyUserInterface.md#36257)
- [Highlighting form fields and navigational components](JS_Dev_ModifyUserInterface.md#60825)
- [Setting up a presentation](JS_Dev_ModifyUserInterface.md#94930)
- [Numbering pages](JS_Dev_ModifyUserInterface.md#36794)
- [Creating buttons](JS_Dev_ModifyUserInterface.md#23421)
...
In addition to adding new or existing bookmarks as you learned in [Creating bookmarks](JS_Dev_ModifyUserInterface.md#64295), you can also delete a bookmark and its children by invoking its `remove` method. The following line of code removes all bookmarks from the document:
...
If a PDF document contains text beginning with [http://](http://), such as [http://www.example.com](http://www.example.com), you can convert all such instances to links with URL actions by invoking the Doc object `addWeblinks` method. The method returns an integer representing the number of text instances converted, as shown in the code below:
...
1. Add navigation links to the document
...
You can use JavaScript to customize the viewing mode when setting up presentations. The `app` object `fs` property may be used to set the viewing mode. (Media clips can also be played in full screen, see the Example [Play a clip in full screen](JS_Dev_DigitalMedia.md#90837).)
...
In addition, you can set up a default page transition through the FullScreen object `defaultTransition` property, as the Example [Setting full screen preferences and resetting them](JS_Dev_ModifyUserInterface.md#68950) demonstrates.
