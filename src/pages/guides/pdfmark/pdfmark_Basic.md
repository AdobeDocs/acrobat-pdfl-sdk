...
This chapter describes the basic pdfmark features. In general, the key–value pairs used as arguments for pdfmark follow closely the key–value pairs that appear in the PDF file. For a description of the PDF file format, see the [PDF Reference](https://www.adobe.com/go/pdfreference).

The following features are described in this chapter:

- [Annotations (ANN)](pdfmark_Basic.md#50454556_26704)
- [Articles (ARTICLE)](pdfmark_Basic.md#50454556_30435)
- [Bookmarks (OUT)](pdfmark_Basic.md#50454556_44268)
- [Document Info dictionary (DOCINFO)](pdfmark_Basic.md#50454556_33876)
- [Document open options (DOCVIEW)](pdfmark_Basic.md#50454556_62753)
- [Embedded file content (EMBED)](pdfmark_Basic.md#50454556_76251)
- [Graphics encapsulation (BP, EP, SP)](pdfmark_Basic.md#50454556_78698)
- [Marked content (MP, DP, BMC, BDC, EMC)](pdfmark_Basic.md#50454556_41438)
- [Metadata (Metadata)](pdfmark_Basic.md#50454556_82967)
- [Named images (NI)](pdfmark_Basic.md#50454556_14682)
- [Page crops (PAGE, PAGES)](pdfmark_Basic.md#50454556_42582)
- [Page label and plate color (PAGELABEL)](pdfmark_Basic.md#50454556_78357)
- [Transparency (SetTransparency)](pdfmark_Basic.md#50454556_93185)

Other pdfmark features are defined in other chapters of this document.

## Annotations (ANN)

PDF supports several types of annotations. The properties of each annotation are specified in an annotation dictionary containing various key–value pairs. The [PDF Reference](https://www.adobe.com/go/pdfreference) describes all the types of annotations, and their required and optional dictionary entries.
...
The following table lists optional keys that are common to all annotations. Specific annotation types have additional keys that they use. See the [PDF Reference](https://www.adobe.com/go/pdfreference) for complete information.
...
| --- | --- | --- |
| Key | Type | Semantics |
| Action(PDF key = `A` ) | name or dictionary | An action to be performed when the annotation is activated. See Actions for details.<br><br>For links, this key is not permitted if the `Dest` key is present. |
| AP | dictionary | An appearance dictionary specifying how the annotation is presented visually. See the [PDF Reference](https://www.adobe.com/go/pdfreference) for details. |
| AS | name | The annotation’s appearance state. See the [PDF Reference](https://www.adobe.com/go/pdfreference) for details. |
...
| Title(PDF key = `T` ) | string | The text label to be displayed in the title bar of the annotation’s pop-up window when open and active<br><br>The encoding and character set used is either PDFDocEncoding (as described in the [PDF Reference](https://www.adobe.com/go/pdfreference) ) or Unicode. If Unicode, the string must begin with <FEFF>. For example, the string “ABC” is represented as (ABC) in PDFDocEncoding and <FEFF004100420043> in Unicode. `Title` has a maximum length of 255 PDFDocEncoding characters or 126 Unicode values, although a practical limit of 32 characters is advised so that it can be read easily in the Acrobat viewer. |

[Text annotations (notes)](pdfmark_Basic.md#50454556_16273) and Links describe the syntax for two of the original and most commonly used annotation types in more detail.
...
| --- | --- | --- |
| Key | Type | Semantics |
| Contents | string | Required. Contains the note’s text string. The maximum length of the `Contents` string is 65,535 characters. The encoding and character set used is the PDFDocEncoding (described in the [PDF Reference](https://www.adobe.com/go/pdfreference) ) or Unicode. If Un |
...
1. Simple link (old style, compatible with all Distiller application versions)
...
1. Link that launches another file
...
1. Custom link action (URI link for the Acrobat WebLink plug-in)
...
1. Custom link action (named action)
...
One useful type of annotation is the widget annotation. Widgets are used by PDF interactive forms to represent the appearance of fields and to manage user interactions. See the [PDF Reference](https://www.adobe.com/go/pdfreference) for detailed information on using interactive forms.

For examples of using widget annotations to create interactive forms, see [Define the Widget annotations, which are also field dictionaries for this form](pdfmark_Examples.md#50454555_89212).
...
1. Create text for the article “Now is the Time”
...
1. Article containing two beads
...
| Count | integer | Required if the bookmark has subordinate bookmarks, omitted otherwise. This key’s absolute value is the number of bookmarks immediately subordinate—that is, excluding subordinates of subordinates. If the value is positive, the bookmark is open, revealing its subordinates; if negative, the bookmark is closed, hiding its subordinates.<br><br>- This differs from the PDF `Count` key, which represents the total number of open descendants at all lower levels of the outline hierarchy. |
| Color | array | Optional. The bookmark’s color. The value is an array containing three numbers (red, green, and blue), each of which must be between 0 and 1, inclusive, specifying a color in the DeviceRGB color space. (See the [PDF Reference](https://www.adobe.com/go/pdfreference) for a description of this color space.) |
| F | integer | Optional. The style of the bookmark. Four styles are implemented:<br><br>- 0 — Plain (the default)<br>- 1 — Italic<br>- 2 — Bold<br>- 3 — Bold and Italic |
...
| Producer | string | Optional. The name of the application that converted the document from its native form to PDF.<br><br>> **Note**<br>><br>> Distiller ignores the setting of this attribute. |
...
For information on the file-embedding Distiller command line option (Windows and UNIX ) and user preference (Mac), see the [Distiller API Reference](../distiller/index.md).
...
1. Using BP and EP pdfmarks to define button faces for forms
...
The `PAGELABEL` pdfmark allows specification of the *page label* for a given page. Page labels can be strings like “iv” or “3-24”, and do not necessarily correspond to the actual page numbers, which run consecutively. See the [PDF Reference](https://www.adobe.com/go/pdfreference) for details.
...
[Transparency group XObject](pdfmark_Basic.md#50454556_89255)

[Soft mask dictionaries](pdfmark_Basic.md#50454556_17842)

[Soft mask images](pdfmark_Basic.md#50454556_57844)

See the [PDF Reference](https://www.adobe.com/go/pdfreference) for complete information.
